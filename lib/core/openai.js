...
/**
 * @deprecated Use createProvider() + provider.complete() directly. Kept for backwards compat.
 */
function getClient() {
...
async function callProvider({ prompt, model, provider }) {
...
/** @deprecated Alias for callProvider — kept for backwards compatibility. */
...
async function generateCommitMessageFull(
  ...
  let raw = "";
  try {
    raw = await callProvider({ prompt: basePrompt, model, provider: p });
  } catch {
    return "";
  }
