...
/**
 * Clamp to maxLen without splitting a word: back up to the last space, or hard-truncate
 * a single long token (no spaces).
 * @param {string} s
 * @param {number} maxLen
 * @returns {string}
 */
function truncateSubjectAtWordBoundary(s, maxLen) {
  const t = (s || "").trim();
  if (t.length <= maxLen) {
    return t;
  }
...
function normalizeSubject(subject) {
  ...
  if (s.length > SUBJECT_MAX_LENGTH) {
    s = truncateSubjectAtWordBoundary(s, SUBJECT_MAX_LENGTH);
  }
  return s;
}
...
function buildFallbackSubject(files) {
  ...
  let s =
    summary.length > SUBJECT_MAX_LENGTH
      ? truncateSubjectAtWordBoundary(summary, SUBJECT_MAX_LENGTH)
      : summary;
  if (s && !/^[A-Z]/.test(s)) s = s.charAt(0).toUpperCase() + s.slice(1);
  return s;
}
